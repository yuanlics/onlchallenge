...
#                 nn.Sigmoid()  # DEBUG 不需要sigmoid
        ...
        self.device = device
        self.MIN_ACTION = 0.05  # DEBUG 限定最小估计
        self.MAX_ACTION = 20  # DEBUG 限定最大估计
...
#         value = self.critic(state)
        ...
        action_mean = state.flatten()[0].detach() + action_mean
...
#         action_logprobs = dist.log_prob(action)
        ...
        return torch.clamp(action, self.MIN_ACTION, self.MAX_ACTION).detach(), action_logprobs, value

    def evaluate(self, state, action):
        action_mean = self.actor(state)
        
        # DEBUG
        action_mean = torch.clamp(state.flatten()[0].detach() + action_mean, self.MIN_ACTION, self.MAX_ACTION)
